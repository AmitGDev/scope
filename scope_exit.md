...
To make this safe, the callable must be nothrow-invocable. There is exactly one class-level invariant enforcing this, checked unconditionally for every instantiation of `scope_exit<F>`:

```cpp
static_assert(std::is_nothrow_invocable_v<F&>);
```

This single assert backs the destructor's call above, and also backs the recovery call used by both constructors' failure paths (see "Ownership and Construction" below).

The const-lvalue constructor additionally needs the callable to be invocable through a `const F&`, specifically because its failure path invokes the callable that way inside a `catch` block. This is **not** a second `static_assert` - it is a `requires` clause attached only to that one constructor:
...
The distinction matters. A `static_assert` at class scope would reject the entire type for every instantiation, regardless of which constructor is actually used. A `requires` clause only removes *that particular overload* from consideration. A mutable lambda (non-const `operator()`) is not invocable through `const F&`, so it simply cannot be constructed via this copy-constructor overload - but it remains fully usable through the `F&&` constructor, which carries no such constraint.
...
while a potentially throwing callable is rejected outright by the class-level assert:
...
The `requires` clause exists purely for diagnostics: without it, attempting to move a `scope_exit<F>` whose `F` isn't move-constructible would fail deep inside the initializer list with a confusing error. With it, overload resolution fails earlier and cleanly, pointing directly at the constraint that wasn't satisfied.
...
Without this guide, constructing a `scope_exit` from an lvalue callable via class template argument deduction would deduce a reference type for `F`, and the object would store a reference member instead of a value - reintroducing exactly the dangling-reference risk the guard exists to avoid. The explicit guide forces decay to a value type regardless of how the argument is passed.
